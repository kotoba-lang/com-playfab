(ns playfab.main
  "Kotodama WASM entrypoint for the Playfab clean-room actor (L4) — Clojure port.
  ...
  facts (`playfab.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "playfab")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"playfab.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "playfab-compat" :tier tier :entities entities} 200])
...
(defn start! [] :playfab-compat/ready)
